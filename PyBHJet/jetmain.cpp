...
void jetmain(BhJetClass& bhjet, double* ear, int ne, double* photeng, double* photspec, JetOutput& output) {
    ...
    int nz;     								//total number of zones    // updated for the new grid 
    int nel = 70;
    ...
    double dlgz = 0.03;    // updated for the new grid 
    ...
    // grid_pars grid;								//structure with grid parameters    // updated for the new grid 
...
//STEP 4: JET BASE EQUIPARTITION CALCULATIONS AND SETUP
    ...
    // grid.nz = nz; // updated for the new grid
    // grid.cut = 0;
    // // grid.zcut = 1.e3*Rg;
    // grid.zcut = z_diss; 
    ...
    // --------- build the grid for the jet based on the value of z_diss: ---------
...
//STEP 5: TOTAL JET CALCULATIONS, LOOPING OVER EACH SEGMENT OF THE JET
    ...
    for(int i=0;i<nz;i++){
        ...
        // jetgrid(i,grid,jet_dyn,zone.r,zone.delz,z);	// updated for the new grid: 

        //this is replacing jetgrid calculation
        if(i<N1) {
