...
class BHJetModel(Function1D, metaclass=FunctionMeta):
    ...
    def evaluate(self, x, Mbh, theta,  dist, redsh, jetrat, r_0, z_diss, z_acc, z_max, t_e, 
        ...
        if self._cached_params is None or params_vec != self._cached_params:
            ...
            #fucky, but just going to have to be right now 
            self.bhjet.cutoff_type = int(self.cutoff_type)
            t0 = time.perf_counter()

            self.bhjet.set_parameters(list(params_vec))
            sig_acc_lin = 10.0 ** self.sig_acc.value
            self.bhjet.set_parameter("sig_acc", sig_acc_lin)

            # print("ThreeML sampled log10 sig_acc:", self.sig_acc.value)
            # print("linear sig_acc sent to C++:", sig_acc_lin)
            # print("C++ sig_acc after set:", self.bhjet.get_parameter("sig_acc"))
        
            self._t_set += time.perf_counter() - t0
            ...
            self._t_out_array += time.perf_counter() - t0
    ...
    # interpolation from BHJet energy grid to 3ml x points : 
            ...
            t0 = time.perf_counter()
            native_energy, native_flux_mjy = self.bhjet.get_total_arrays()
            self._t_arrays += time.perf_counter() - t0
        ...
        # convert energy to expected keV output - tries to catch for different cases of incorrect returns
            ...
            self._t_convert += time.perf_counter() - t0 



        t0 = time.perf_counter()
        x = np.asarray(x, dtype=float)
        y = np.full_like(x, 1e-99, dtype=float) #this was an issue before 
        ...
        y[bad] = 1e-99
        self._t_interp += time.perf_counter() - t0
        return y

    

    def get_detail(self,name,e_min_keV=1e-9,e_max_keV=1e8,n_eval=200,infosw=3):
        ...
        old_cache = getattr(self, "_cached_params", None)
