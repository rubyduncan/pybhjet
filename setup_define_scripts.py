# import astromodels  # Temporarily disabled while the threeML environment is unavailable.
import yaml
import numpy as np
import pandas as pd
...
_PYBHJET_MODULE_DIR = Path(__file__).resolve().parent / "PyBHJet"
if str(_PYBHJET_MODULE_DIR) not in sys.path:
    sys.path.insert(0, str(_PYBHJET_MODULE_DIR))
...
def plot_sed_from_components(
    ...
    **kwargs,
    ...
    """Plot YAML-style component expressions with the composable plotting API.

    ``sed_components`` maps legend labels to expression strings, for example
    ``{"Jet": "jet", "X-ray": "gal_ext*(zpcfabs*jet+pexmon)"}``.
    Model construction remains in :func:`build_spectrum`; this is only a thin,
    notebook-friendly bridge to :func:`plotting.plot_sed`.
    """
    models = {
        label: build_spectrum(expression, model_components)
        for label, expression in sed_components.items()
    }
    return plot_sed(
        data=data_dict,
        model_components=model_components,
        model_expressions=models,
        xray_path=xray_path,
        ax=ax,
        style=style,
...
def hz_plot_xylike_data(xy, ax=None, label=None, color_data="k", color_model="r"):
    """Compatibility wrapper for :func:`plotting.plot_xylike`."""
    plot_xylike(xy, ax=ax, label=label, color=color_data)
    return (ax or plt.gca()).figure, (ax or plt.gca())


def lum_plot_xylike_data(model_components, xy, ax=None, label=None, color_data="k", color_model="r"):
    """Compatibility wrapper for a luminosity-space :func:`plot_xylike`."""
    plot_xylike(xy, ax=ax, label=label, color=color_data, luminosity=True,
                model_components=model_components)
    return (ax or plt.gca()).figure, (ax or plt.gca())
...
def hz_plot_model_space_sed(
    ...
    style = PlotStyle(data_colors=dict(zip(data_keys, data_colors)), model_linewidth=model_lw)
    ax = plot_sed_from_components(data_dict, model_components, sed_components_expr,
                                  xray_path=xray_path, style=style,
                                  energy_range=(e_min_keV, e_max_keV), n_points=n_points)
    return ax.figure, ax


def add_xray_to_ax(ax,path,model_components,*,
    y_col=2,
    yerr_col=3,
    fmt="D",
    markersize=8,
    ...
    **errorbar_kwargs,
    ...
    """Compatibility wrapper for :func:`plotting.plot_xray_file`."""
    return plot_xray_file(path, ax=ax, model_components=model_components,
                          luminosity=lum, y_col=y_col, yerr_col=yerr_col,
                          fmt=fmt, ms=markersize, alpha=alpha, zorder=zorder,
                          label=label, color=color, **errorbar_kwargs)
...
def lum_plot_all_data_w_fake_xray(
    ...
    style = PlotStyle(data_colors=dict(zip(data_keys, data_colors)), model_linewidth=model_lw)
    ax = plot_sed_from_components(data_dict, model_components, sed_components_expr,
                                  xray_path=xray_path, style=style, luminosity=True,
                                  energy_range=(e_min_keV, e_max_keV), n_points=n_points)
    return ax.figure, ax
...
def hz_eval_and_plot_sed(model, data_dict, components, sed_components_expr=None,
                         e_min_keV=1e-9, e_max_keV=1e3, n_points=1000,
                         verbose=True, radio_key='rad', ir_key='ir', uv_key='uv'):
    ...
    fig, ax = hz_plot_model_space_sed(
        data_dict, components, sed_components_expr or {},
